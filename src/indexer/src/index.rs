...
use entry::{Entry, HeaderValue, InscriptionEntry, InscriptionNumber, MyOutPoints, OutPointValue, RuneEntry, RuneIdValue, SatPointValue, SequenceNumbers, TxidValue};
use event::Events;
use ic_stable_structures::{StableCell, StableBTreeMap};
use ordinals::{Charm, SatPoint};
...
use crate::{
  chain::Chain, config::Config, inscriptions::{envelope::ParsedEnvelope, Inscription, InscriptionId, InscriptionQuery, InscriptionResp}, memory::{
    get_virtual_memory, VMemory, CONFIG_MEMORY_ID, HEIGHT_TO_BLOCK_HEADER_MEMORY_ID, HEIGHT_TO_EVENTS_MEMORY_ID, HEIGHT_TO_LAST_SEQUENCE_NUMBER_MEMORY_ID, HOME_INSCRIPTIONS_MEMORY_ID, INSCRIPTION_ID_TO_SEQUENCE_NUMBER_MEMORY_ID, INSCRIPTION_NUMBER_TO_SEQUENCE_NUMBER_MEMORY_ID, OUTPOINT_TO_UTXO_ENTRY_MEMORY_ID, RUNE_ID_TO_RUNE_ENTRY_MEMORY_ID, SAT_TO_SATPOINT_MEMORY_ID, SAT_TO_SEQUENCE_NUMBERS_MEMORY_ID, SCRIPT_PUBKEY_TO_OUTPOINTS_MEMORY_ID, SEQUENCE_NUMBER_TO_CHILDRENS_MEMORY_ID, SEQUENCE_NUMBER_TO_INSCRIPTION_ENTRY_MEMORY_ID, SEQUENCE_NUMBER_TO_RUNE_ID_MEMORY_ID, SEQUENCE_NUMBER_TO_SATPOINT_MEMORY_ID, STATISTIC_TO_COUNT_MEMORY_ID, TRANSACTION_ID_TO_TRANSACTION_MEMORY_ID
  }, re, timestamp, unbound_outpoint, Result
};
...
pub fn next_block(index: &Index) -> (u32, Option<BlockHash>) {
  mem_latest_block()
    .map(|(height, prev_blockhash)| (height + 1, Some(prev_blockhash)))
    .unwrap_or((index.chain().first_inscription_height(), None))
}

pub fn mem_insert_sat_to_sequence_numbers(sat: u64, seq: u32) -> bool {
...
pub fn mem_get_first_seq_of_sat_to_sequence_numbers(sat: u64) -> Option<u32> {
  SAT_TO_SEQUENCE_NUMBERS.with(|m| m.borrow().get(&sat).map(|seqs| seqs.0[0]))
}

pub fn mem_insert_sequence_number_to_childrens(seq: u32, seq_children: u32) -> bool {
...
pub fn mem_get_sequence_number_to_childrens(seq: u32) -> Option<SequenceNumbers> {
  SEQUENCE_NUMBER_TO_CHILDRENS.with(|m| m.borrow().get(&seq))
}

pub fn mem_insert_script_pubkey_to_outpoints(script_pubkey: Vec<u8>, outpoint: OutPoint) -> bool {
...
pub fn mem_remove_script_pubkey_to_outpoints(script_pubkey: Vec<u8>, outpoint: &OutPoint) -> bool {
  SCRIPT_PUBKEY_TO_OUTPOINTS.with(|m| {
    let mut map = m.borrow_mut();
    let mut outpoints = map.get(&script_pubkey).unwrap_or_default();
    if !outpoints.contains(outpoint) {
...
pub fn mem_get_sequence_number_to_inscription_entry(seq: u32) -> Option<InscriptionEntry> {
  SEQUENCE_NUMBER_TO_INSCRIPTION_ENTRY.with(|m| m.borrow().get(&seq))
}

pub fn mem_insert_sequence_number_to_inscription_entry(seq: u32, entry: InscriptionEntry) {
  SEQUENCE_NUMBER_TO_INSCRIPTION_ENTRY.with(|m| m.borrow_mut().insert(seq, entry));
}

pub fn mem_get_next_sequence_of_sequence_number_to_inscription_entry()-> u32 {
  SEQUENCE_NUMBER_TO_INSCRIPTION_ENTRY.with(|m| m.borrow().iter().next_back().map(|(seq, _)| seq + 1).unwrap_or(0))
}

pub fn mem_get_inscription_id_to_sequence_number(id: &InscriptionId) -> Option<u32> {
  INSCRIPTION_ID_TO_SEQUENCE_NUMBER.with(|m| m.borrow().get(id))
}

pub fn mem_insert_inscription_id_to_sequence_number(id: InscriptionId, seq: u32) {
  INSCRIPTION_ID_TO_SEQUENCE_NUMBER.with(|m| m.borrow_mut().insert(id, seq));
}

pub fn mem_insert_transaction_id_to_transaction(id: TxidValue, tx: Vec<u8>) {
  TRANSACTION_ID_TO_TRANSACTION.with(|m| m.borrow_mut().insert(id, tx));
}

pub fn mem_get_transaction_id_to_transaction(id: TxidValue) -> Option<Vec<u8>> {
  TRANSACTION_ID_TO_TRANSACTION.with(|m| m.borrow().get(&id))
}

pub fn mem_insert_inscription_number_to_sequence_number(num: &InscriptionNumber, seq: u32) {
  INSCRIPTION_NUMBER_TO_SEQUENCE_NUMBER.with(|m| m.borrow_mut().insert(num.clone(), seq));
}

pub fn mem_get_inscription_number_to_sequence_number(num: &InscriptionNumber) -> Option<u32> {
  INSCRIPTION_NUMBER_TO_SEQUENCE_NUMBER.with(|m| m.borrow().get(num))
}

pub fn mem_insert_home_inscriptions(seq: u32, id: InscriptionId) {
  HOME_INSCRIPTIONS.with(|m| m.borrow_mut().insert(seq, id));
}

pub fn mem_pop_first_home_inscriptions() -> Option<(u32,InscriptionId)> {
  HOME_INSCRIPTIONS.with(|m| m.borrow_mut().pop_first())
}

pub fn mem_get_home_inscriptions_len() -> u64 {
  HOME_INSCRIPTIONS.with(|m| m.borrow().len())
}

pub fn mem_insert_height_to_events(height: u32, events: Events) {
  HEIGHT_TO_EVENTS.with(|m| m.borrow_mut().insert(height, events));
}

pub fn mem_remove_height_to_events(height: u32) {
  HEIGHT_TO_EVENTS.with(|m| m.borrow_mut().remove(&height));
}

pub fn mem_insert_statistic_to_count(statistic: Statistic, count: u64) {
  STATISTIC_TO_COUNT.with(|m| m.borrow_mut().insert(statistic.key(), count));
}

pub fn mem_increment_statistic(statistic: Statistic, n:u64) {
...
pub fn mem_get_statistic_count(statistic: Statistic) -> u64 {
  STATISTIC_TO_COUNT.with(|m| m.borrow().get(&statistic.key()).unwrap_or_default())
}

pub fn mem_remove_outpoint_to_utxo_entry(outpoint: OutPointValue) -> Option<UtxoEntry> {
  OUTPOINT_TO_UTXO_ENTRY.with(|m| m.borrow_mut().remove(&outpoint))
}

pub fn mem_get_outpoint_to_utxo_entry(outpoint: OutPointValue) -> Option<UtxoEntry> {
  OUTPOINT_TO_UTXO_ENTRY.with(|m| m.borrow().get(&outpoint))
}

pub fn mem_insert_outpoint_to_utxo_entry(outpoint: OutPointValue, utxo_entry: UtxoEntry) {
  OUTPOINT_TO_UTXO_ENTRY.with(|m| m.borrow_mut().insert(outpoint, utxo_entry));
}

pub fn mem_insert_sat_to_satpoint(sat: u64, satpoint: SatPointValue) {
  SAT_TO_SATPOINT.with(|m| m.borrow_mut().insert(sat, satpoint));
}

pub fn mem_insert_height_to_last_sequence_number(height: u32, seq: u32) {
  HEIGHT_TO_LAST_SEQUENCE_NUMBER.with(|m| m.borrow_mut().insert(height, seq));
}

pub fn mem_get_height_to_last_sequence_number(height: u32) -> Option<u32> {
  HEIGHT_TO_LAST_SEQUENCE_NUMBER.with(|m| m.borrow().get(&height))
}

pub fn mem_insert_sequence_number_to_satpoint(seq: u32, satpoint: SatPointValue) {
  SEQUENCE_NUMBER_TO_SATPOINT.with(|m| m.borrow_mut().insert(seq, satpoint));
}

pub fn mem_get_sequence_number_to_satpoint(seq: u32) -> Option<SatPointValue> {
  SEQUENCE_NUMBER_TO_SATPOINT.with(|m| m.borrow().get(&seq))
}

pub fn mem_get_sequence_number_to_rune_id(seq: u32) -> Option<RuneIdValue> {
  SEQUENCE_NUMBER_TO_RUNE_ID.with(|m| m.borrow().get(&seq))
}

pub fn mem_get_rune_id_to_rune_entry(rune_id: RuneIdValue) -> Option<RuneEntry> {
  RUNE_ID_TO_RUNE_ENTRY.with(|m| m.borrow().get(&rune_id))
}
