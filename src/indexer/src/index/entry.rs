...
use ordinals::{Pile, Rune, RuneId, Sat, SatPoint, SpacedRune, Terms};
use serde::{Deserialize, Serialize};

use crate::{inscriptions::InscriptionId, runes::MintError};
...
#[derive(Debug, Clone)]
pub struct SequenceNumbers(pub Vec<u32>);
...
impl SequenceNumbers {
  pub fn push(&mut self, sequence_number: u32) {
    self.0.push(sequence_number);
  }
  ...
  pub fn contains(&self, sequence_number: u32) -> bool {
    self.0.contains(&sequence_number)
  }
}

impl Storable for SequenceNumbers {
  fn to_bytes(&self) -> Cow<[u8]> {
    let vec = bincode::serialize(&self.0).unwrap();
    Cow::Owned(vec)
  }
...
pub struct InscriptionNumber(i32);

impl From<i32> for InscriptionNumber {
  fn from(n: i32) -> Self {
    Self(n)
  }   
}

impl From<InscriptionNumber> for i32 {
  fn from(n: InscriptionNumber) -> Self {
    n.0
  } 
}

impl Storable for InscriptionNumber {
  fn to_bytes(&self) -> Cow<[u8]> {
    Cow::Owned(self.0.to_le_bytes().to_vec())
  }

  fn from_bytes(bytes: Cow<[u8]>) -> Self {
    Self(i32::from_le_bytes(bytes.as_ref().try_into().unwrap()))
  }
...
impl Storable for InscriptionEntry {
  fn to_bytes(&self) -> Cow<[u8]> {
    let vec = bincode::serialize(self).unwrap();
    Cow::Owned(vec)
  }
...
#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord)]
pub struct MyOutPoints {
  pub outpoints: Vec<OutPoint>,
}

impl Default for MyOutPoints {
  fn default() -> Self {
    Self {
      outpoints: Vec::new(),
    }
...
impl MyOutPoints {
  pub fn push(&mut self, outpoint: OutPoint) {
    self.outpoints.push(outpoint);
  }

  pub fn pop(&mut self) -> Option<OutPoint> {
    self.outpoints.pop()
  }
  ...
  where
    F: FnMut(&OutPoint) -> bool,
  {
    self.outpoints.retain(f);
  }
    
}

impl Storable for MyOutPoints {
  fn to_bytes(&self) -> Cow<[u8]> {
    let vec = bincode::serialize(&self.outpoints).unwrap();
    Cow::Owned(vec)
  }

  fn from_bytes(bytes: Cow<[u8]>) -> Self {
    Self {
      outpoints: bincode::deserialize(&bytes).unwrap(),
    }
  }
