...
use bitcoin::{consensus::Encodable, OutPoint, Transaction, Txid};
use ordinals::{Charm, Height, Sat, SatPoint};

use crate::{
  index::{entry::{Entry, InscriptionEntry, InscriptionNumber, SatRange}, event::{Event, Events}, mem_get_inscription_id_to_sequence_number, mem_get_sequence_number_to_inscription_entry, mem_insert_home_inscriptions, mem_insert_inscription_id_to_sequence_number, mem_insert_inscription_number_to_sequence_number, mem_insert_sat_to_sequence_numbers, mem_insert_sequence_number_to_childrens, mem_insert_sequence_number_to_inscription_entry, mem_insert_transaction_id_to_transaction, mem_pop_first_home_inscriptions, utxo_entry::{ParsedUtxoEntry, UtxoEntryBuf}, Index}, inscriptions::{envelope::ParsedEnvelope, InscriptionId}, unbound_outpoint, Result
};
...
impl InscriptionUpdater {
    ...
    let jubilant = self.height >= index.chain().jubilee_height();
    ...
    for (input_index, txin) in tx.input.iter().enumerate() {
      ...
      for (sequence_number, old_satpoint_offset) in transferred_inscriptions {
        ...
        let inscription_id = mem_get_sequence_number_to_inscription_entry(sequence_number).expect("sequence number not found").id;
        ...
        let curse = if inscription.payload.unrecognized_even_field {
          ...
          if *count > 1 {
            ...
            let initial_inscription_sequence_number =
              mem_get_inscription_id_to_sequence_number(id).expect("inscription id not found");

            let entry = mem_get_sequence_number_to_inscription_entry(initial_inscription_sequence_number).expect("sequence number not found");
    ...
    if index.index_transactions && has_new_inscriptions {
      ...
      mem_insert_transaction_id_to_transaction(txid.store(), self.transaction_buffer.clone());
  ...
  fn update_inscription_location(
    ...
    let (unbound, sequence_number) = match flotsam.origin {
      Origin::Old {
        ...
        if op_return {
          let entry = mem_get_sequence_number_to_inscription_entry(sequence_number).expect("sequence number not found");
          let mut charms = entry.charms;
          Charm::Burned.set(&mut charms);

          mem_insert_sequence_number_to_inscription_entry(
            sequence_number,
            InscriptionEntry {charms, ..entry},
          );
        }
        self.events.push(Event::InscriptionTransferred {
      ...
      Origin::New {
        ...
        mem_insert_inscription_number_to_sequence_number(
          &InscriptionNumber::from(inscription_number),
          sequence_number,
        );
        ...
        if let Some(Sat(n)) = sat {
          mem_insert_sat_to_sequence_numbers(n, sequence_number);
        }

        let parent_sequence_numbers = parents
          .iter()
          .map(|parent| {
            let parent_sequence_number = mem_get_inscription_id_to_sequence_number(parent).expect("inscription id not found");
            mem_insert_sequence_number_to_childrens(parent_sequence_number, sequence_number);
            Ok(parent_sequence_number)
          })
          .collect::<Result<Vec<u32>>>()?;
        
        self.events.push(Event::InscriptionCreated {
          block_height: self.height,
          charms,
          inscription_id,
        ...
        mem_insert_inscription_id_to_sequence_number(inscription_id, sequence_number);

        if !hidden {
          mem_insert_home_inscriptions(sequence_number, inscription_id);
          if self.home_inscription_count == 100 {
            mem_pop_first_home_inscriptions();
          } else {
