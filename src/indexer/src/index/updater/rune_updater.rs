...
use crate::index::event::{Event, Events};
use crate::index::lot::Lot;
use crate::into_usize::IntoUsize;
use crate::Result;
use anyhow::anyhow;
use crate::index::entry::{ChangeRecordRune, Entry, RuneBalance, RuneBalances, RuneEntry};
...
pub(super) struct RuneUpdater {
  ...
  pub(super) runes: u64,
  pub(super) change_record: ChangeRecordRune,
...
impl RuneUpdater {
  pub(super) async fn index_runes(
    ...
    for (vout, balances) in allocated.into_iter().enumerate() {
      ...
      for (id, balance) in balances {
        ...
        self.events.push(Event::RuneTransferred {
          amount: balance.n(),
    ...
    for (id, amount) in burned {
      *self.burned.entry(id).or_default() += amount;
      ...
      self.events.push(Event::RuneBurned {
        amount: amount.n(),
        block_height: self.height,
        rune_id: id,
        txid,
  ...
  pub(super) fn update(mut self) -> Result {
    ...
    crate::index::mem_insert_change_record_rune(self.height, self.change_record);
